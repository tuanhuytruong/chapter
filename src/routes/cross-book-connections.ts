...
async function canGenerate(ownerId: string) {
  ...
  return entitlement.active && limit !== "unavailable";
}
...
crossBookConnectionsRouter.get("/current", async (req: Request, res: Response) => {
  try {
    ...
    res.json({ connection, sourceBookCount: new Set(sources.map((source) => source.bookId)).size, sourceSessionCount: sources.filter((source) => source.sourceType === "log").length, hasSource: hasConnectionSource(sources), available: entitlement.active && limit !== "unavailable" && (limit === "unlimited" || current.used + current.reserved < limit), usage: { used: current.used, reserved: current.reserved, limit } });
