  ...
  return <section className="mt-3 rounded-2xl border border-natural-border bg-natural-cream/50 p-3 sm:p-4">
    <div className="flex min-h-11 items-center gap-1">
      <button type="button" onClick={() => setOpen((value) => !value)} aria-expanded={open} aria-controls={regionId} className="flex min-h-11 min-w-0 flex-1 items-center gap-2 text-left">
        <ScanSearch className="h-4 w-4 shrink-0 text-natural-sage" /><span className="flex-1 text-sm font-bold uppercase tracking-wider text-natural-sage"><GlossaryLabel term="ReadingLens" language={language} /></span>
      </button>
      ...
      <button type="button" onClick={() => setOpen((value) => !value)} aria-expanded={open} aria-controls={regionId} aria-label="Toggle Reading Lens details" className="flex min-h-8 min-w-8 shrink-0 items-center justify-center rounded-full text-natural-stone transition hover:bg-natural-bg hover:text-natural-dark focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-natural-sage/40"><ChevronDown className={`h-4 w-4 transition-transform ${open ? "rotate-180" : ""}`} /></button>
    </div>
    <p className="mt-1 text-sm leading-relaxed text-natural-dark">{lens.analyst_summary}</p>
    {a.durableInsights.length > 0 && <ul className="mt-2 space-y-1 text-sm leading-relaxed text-natural-stone">{a.durableInsights.slice(0, 3).map((item, i) => <li key={i}>• {item}</li>)}</ul>}
    {open && <div id={regionId} role="region" aria-label="Reading Lens details" className="mt-4 space-y-4 border-t border-natural-border pt-3 text-sm leading-relaxed text-natural-dark">
...
function LensList({ title, items, muted }: { title: ReactNode; items: ReactNode[]; muted?: boolean }) { if (!items.length) return null; return <div><h4 className="mb-1 text-sm font-bold uppercase tracking-wider text-natural-sage">{title}</h4><ul className={`space-y-1 ${muted ? "text-natural-stone" : ""}`}>{items.map((item, i) => <li key={i}>• {item}</li>)}</ul></div>; }
