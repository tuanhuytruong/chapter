...
export default function BookWiki({ bookId, totalPages, canEdit }: { bookId: string; totalPages: number; canEdit: boolean }) {
  ...
  const fetchReader = useCallback(async () => {
    try {
      ...
      setSessions(rawSessions?.map((row: any) => {
        ...
        return { ...analysis, id: row?.log_id || analysis?.id, page_start: row?.page_start ?? analysis?.page_start, page_end: row?.page_end ?? analysis?.page_end, title: analysis?.session_title || analysis?.title, summary: analysis?.close_reading || analysis?.session_summary || analysis?.chunk_summary || analysis?.summary } as ReaderSession;
      }) ?? null);
  ...
  return <div className="space-y-4">
    ...
    {(wiki.reading_path?.length || wiki.thread_map?.length || wiki.entity_map?.length || wiki.connections?.length) ? <ReadingMap wiki={wiki} totalPages={totalPages} status={status?.status || status?.jobStatus} onSession={(id) => setOpenSession(id)} openItem={openItem} setOpenItem={setOpenItem} /> : null}
...
function ReadingMap({ wiki, totalPages, status, onSession, openItem, setOpenItem }: { wiki: BookWikiData; totalPages: number; status?: string; onSession: (id: string) => void; openItem: string | null; setOpenItem: (value: string | null) => void }) {
  ...
  const coverageText = complete ? `Complete · all ${totalPages || covered} pages covered` : totalPages > 0 ? `${covered} of ${totalPages} pages covered` : `${covered} pages covered`;
  ...
  return <section className="space-y-5 rounded-2xl border border-natural-border bg-white/45 p-4 sm:p-5" aria-labelledby="reading-map-heading">
    <div className="rounded-2xl border border-natural-sage/20 bg-natural-sage/5 p-4"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-[10px] font-bold uppercase tracking-wider text-natural-sage">Reading Map</p><h2 id="reading-map-heading" className="mt-1 text-sm font-bold text-natural-dark">The book’s path, held in context</h2><p className="mt-1 text-[11px] leading-relaxed text-natural-stone">{complete ? "The map has reached the book’s ending; threads below show their recorded resolution." : "This is a partial map. It reflects only the pages and sessions processed so far."}</p></div><span className="rounded-full bg-white/75 px-3 py-1 text-[10px] font-bold text-natural-sage">{complete ? "Resolved reading" : "In progress"}</span></div><div className="mt-4"><div className="mb-1 flex justify-between gap-3 text-[10px] text-natural-stone"><span>{coverageText}</span>{totalPages > 0 && <span>{progress}%</span>}</div>{totalPages > 0 && <div className="h-1.5 rounded-full bg-natural-border"><div className="h-full rounded-full bg-natural-sage" style={{ width: `${progress}%` }} /></div>}</div></div>
    {path.length > 0 && <div><div className="mb-2"><h3 className="text-xs font-bold text-natural-dark">Chronological path</h3><p className="mt-1 text-[11px] text-natural-stone">Open a card to return to its saved reading session.</p></div><ol className="grid gap-2 md:grid-cols-2">{path.map((entry, index) => <li key={entry.log_id || `${entry.page_start}-${index}`}><button onClick={() => entry.log_id && onSession(entry.log_id)} disabled={!entry.log_id} className="h-full min-h-28 w-full rounded-2xl border border-natural-border bg-natural-cream/55 p-3 text-left transition hover:border-natural-sage/45 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-natural-sage/50 disabled:cursor-default"><div className="flex items-start justify-between gap-2"><span className="text-[10px] font-bold uppercase tracking-wider text-natural-sage">{String(index + 1).padStart(2, "0")}</span><span className="text-[10px] font-semibold text-natural-stone">{pageLabel(entry.page_start, entry.page_end)}</span></div><h4 className="mt-2 text-xs font-bold text-natural-dark">{entry.title}</h4><p className="mt-1 text-[11px] leading-relaxed text-natural-stone">{entry.turning_point || entry.summary}</p></button></li>)}</ol></div>}
    {(threads.length > 0 || entities.length > 0) && <div className="grid gap-4 md:grid-cols-2"><MapEvidenceGroup title="Threads in motion" description="Each note is anchored to a point on the path." items={threads} itemKey="thread-map" openItem={openItem} setOpenItem={setOpenItem} onSession={onSession} evidenceKey="evolution" /><MapEvidenceGroup title="People, ideas & forces" description="States and appearances remain tied to recorded sessions." items={entities} itemKey="entity-map" openItem={openItem} setOpenItem={setOpenItem} onSession={onSession} evidenceKey="appearances" /></div>}
    {connections.length > 0 && <div className="border-t border-natural-border pt-4"><h3 className="text-xs font-bold text-natural-dark">Connections</h3><p className="mt-1 text-[11px] text-natural-stone">Relationships supported by the recorded map.</p><div className="mt-3 grid gap-2 md:grid-cols-2">{connections.map((connection, index) => { const from = resolveName(connection.from_type, connection.from_id); const to = resolveName(connection.to_type, connection.to_id); return <article key={`${connection.from_type}-${connection.from_id}-${connection.to_type}-${connection.to_id}-${index}`} className="rounded-2xl bg-natural-cream/60 p-3"><p className="text-[10px] font-bold uppercase tracking-wider text-natural-sage">{connection.label}</p><p className="mt-1 text-xs font-semibold text-natural-dark">{from} <span className="font-normal text-natural-stone">↔</span> {to}</p><p className="mt-2 text-[11px] leading-relaxed text-natural-stone">{connection.explanation}</p><p className="mt-2 text-[10px] font-semibold text-natural-sage">Evidence · Page {connection.page_start}</p></article>; })}</div></div>}
  </section>;
}
function MapEvidenceGroup({ title, description, items, itemKey, openItem, setOpenItem, onSession, evidenceKey }: { title: string; description: string; items: Array<MapThread | MapEntity>; itemKey: string; openItem: string | null; setOpenItem: (value: string | null) => void; onSession: (id: string) => void; evidenceKey: "evolution" | "appearances" }) { return <div><h3 className="text-xs font-bold text-natural-dark">{title}</h3><p className="mt-1 text-[11px] text-natural-stone">{description}</p><div className="mt-2 space-y-2">{items.map((item, index) => { const key = `${itemKey}-${item.id || index}`; const open = openItem === key; const label = "label" in item ? item.label : item.name; const state = "status" in item ? properStatus(item.status || "") : (item as MapEntity).current_state; const evidence = evidenceKey === "evolution" ? (item as MapThread).evolution || [] : (item as MapEntity).appearances || []; return <article key={key} className="overflow-hidden rounded-2xl bg-natural-cream/60"><button onClick={() => setOpenItem(open ? null : key)} aria-expanded={open} className="flex min-h-11 w-full items-center justify-between gap-3 px-3 py-2 text-left"><span><span className="block text-xs font-semibold text-natural-dark">{label}</span>{state && <span className="mt-0.5 block text-[10px] text-natural-sage">{state}</span>}</span>{open ? <ChevronDown className="h-4 w-4 shrink-0 text-natural-sage" /> : <ChevronRight className="h-4 w-4 shrink-0 text-natural-stone" />}</button>{open && <div className="space-y-2 border-t border-natural-border px-3 py-3">{evidence.length ? evidence.map((entry, evidenceIndex) => <button key={`${entry.log_id}-${evidenceIndex}`} onClick={() => entry.log_id && onSession(entry.log_id)} disabled={!entry.log_id} className="block w-full border-l-2 border-natural-sage/35 py-1 pl-2 text-left disabled:cursor-default"><span className="text-[10px] font-bold text-natural-sage">Page {entry.page_start}</span><span className="ml-2 text-[11px] leading-relaxed text-natural-stone">{entry.note}</span></button>) : <p className="text-[11px] text-natural-stone">No path evidence is available yet.</p>}</div>}</article>; })}</div></div>; }
