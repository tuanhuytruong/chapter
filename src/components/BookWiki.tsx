...
function MapEvidenceGroup({ title, description, items, itemKey, openItem, setOpenItem, onSession, evidenceKey }: { title: string; description: string; items: Array<MapThread | MapEntity>; itemKey: string; openItem: string | null; setOpenItem: (value: string | null) => void; onSession: (id: string) => void; evidenceKey: "evolution" | "appearances" }) {
  return <div><h3 className="text-xs font-bold text-natural-dark">{title}</h3><p className="mt-1 text-[11px] text-natural-stone">{description}</p><div className="mt-2 space-y-2">{items.map((item, index) => {
    ...
    return <article key={key} className="overflow-hidden rounded-2xl bg-natural-cream/60">
      ...
      {state && <p className="px-3 pb-2 text-[10px] text-natural-sage">{stateTerm ? <GlossaryLabel term={stateTerm} /> : state}</p>}
...
function DrillGroup({ title, items, itemKey, openItem, setOpenItem }: { title: string; items: Record<string, unknown>[]; itemKey: string; openItem: string | null; setOpenItem: (value: string | null) => void }) {
