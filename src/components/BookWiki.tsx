...
export default function BookWiki({ bookId, totalPages, canEdit, onOpenReadingSession, returnTargetId }: { bookId: string; totalPages: number; canEdit: boolean; onOpenReadingSession: (logId: string, sourceId?: string) => void; returnTargetId?: string | null }) {
  ...
  return <div className="space-y-4" onPointerDownCapture={() => { pointerScrollY.current = window.scrollY; }}>
    ...
    <section className="rounded-2xl border border-natural-border bg-natural-cream/45 p-4"><p className="text-sm leading-relaxed text-natural-dark">{wiki.book_so_far || wiki.overview}</p>{wiki.current_position?.label && <p className="mt-3 flex gap-1.5 text-xs text-natural-stone"><Sparkles className="h-3 w-3 shrink-0 text-natural-sage" />{wiki.current_position.label}</p>}</section>
    {wiki.current_reading_state?.summary && <section className="rounded-2xl border border-natural-sage/25 bg-natural-sage/5 p-4"><h2 className="text-xs font-bold uppercase tracking-wider text-natural-sage">At this point</h2><p className="mt-2 text-sm leading-relaxed text-natural-dark">{wiki.current_reading_state.summary}</p>{wiki.next_session_context && <p className="mt-3 border-t border-natural-sage/15 pt-3 text-xs leading-relaxed text-natural-stone">Next session: {wiki.next_session_context}</p>}</section>}
    {(wiki.reading_path?.length || wiki.thread_map?.length || wiki.entity_map?.length || wiki.connections?.length) ? <ReadingMap wiki={wiki} totalPages={totalPages} status={status?.status || status?.jobStatus} language={glossaryLanguage} mode={pageMode} labels={pageLabels} onSession={onOpenReadingSession} returnTargetId={returnTargetId} openItem={openItem} setOpenItem={(value) => preserveScroll(() => setOpenItem(value))} /> : null}
    <section><h2 className="mb-2 text-xs font-bold text-natural-dark">Reading sessions</h2>{readerSessions.length ? <div className="space-y-2">{readerSessions.map((session, index) => { const key = session.id || `session-${index}`; const open = openSession === key; const entities = Array.isArray(session.entities) ? session.entities : []; const threads = Array.isArray(session.threads) ? session.threads : []; return <article key={key} className="overflow-hidden rounded-2xl border border-natural-border bg-white/55"><button type="button" onClick={() => preserveScroll(() => setOpenSession(open ? null : key))} aria-expanded={open} className="flex min-h-12 w-full items-center justify-between gap-3 px-4 text-left"><span><b className="text-sm text-natural-dark">{sessionName(session, glossaryLanguage, pageMode, pageLabels)}</b>{sessionPageLabel(session, glossaryLanguage, pageMode, pageLabels) && <small className="ml-2 text-xs text-natural-stone">{sessionPageLabel(session, glossaryLanguage, pageMode, pageLabels)}</small>}</span>{open ? <ChevronDown className="h-4 w-4 text-natural-sage" /> : <ChevronRight className="h-4 w-4 text-natural-stone" />}</button>{open && <div className="space-y-3 border-t border-natural-border px-4 py-3"><p className="text-sm leading-relaxed text-natural-stone">{text(session.summary) || "Session detail is not available yet."}</p><DrillGroup title="People & concepts" items={entities} itemKey={`${key}-entity`} language={glossaryLanguage} openItem={openItem} setOpenItem={(value) => preserveScroll(() => setOpenItem(value))} /><DrillGroup title="Threads to follow" items={threads} itemKey={`${key}-thread`} language={glossaryLanguage} openItem={openItem} setOpenItem={(value) => preserveScroll(() => setOpenItem(value))} /></div>}</article>; })}</div> : <p className="rounded-2xl border border-dashed border-natural-border p-4 text-center text-xs text-natural-stone">Session details are being prepared.</p>}</section>
    {!sessions && <V1References wiki={wiki} language={glossaryLanguage} preserveScroll={preserveScroll} />}
  </div>;
}
function ReadingMap({ wiki, totalPages, status, language, mode, labels, onSession, returnTargetId, openItem, setOpenItem }: { wiki: BookWikiData; totalPages: number; status?: string; language: GlossaryLanguage; mode: PageMode; labels?: Record<string, number | null>; onSession: (id: string, sourceId?: string) => void; returnTargetId?: string | null; openItem: string | null; setOpenItem: (value: string | null) => void }) {
  ...
  return <section className="space-y-5 rounded-2xl border border-natural-border bg-white/45 p-4 sm:p-5" aria-labelledby="reading-map-heading">
    <div className="rounded-2xl border border-natural-sage/20 bg-natural-sage/5 p-4"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-sm font-bold text-natural-sage">Reading Map</p><h2 id="reading-map-heading" className="mt-1 text-sm font-bold text-natural-dark">The book’s path, held in context</h2><p className="mt-1 text-sm leading-relaxed text-natural-stone">{complete ? "The map has reached the book’s ending; threads below show their recorded resolution." : (mode === "epub_chunk" ? `This is a partial map. It reflects only the ${language === "vi" ? "chương và phiên đọc" : "sections and sessions"} processed so far.` : `This is a partial map. It reflects only the ${language === "vi" ? "trang và phiên đọc" : "pages and sessions"} processed so far.`)}</p></div><span className="rounded-full bg-white/75 px-3 py-1 text-xs font-bold text-natural-sage">{complete ? "Resolved reading" : "In progress"}</span></div><div className="mt-4"><div className="mb-1 flex justify-between gap-3 text-xs text-natural-stone"><span>{coverageText}</span>{totalPages > 0 && <span>{progress}%</span>}</div>{totalPages > 0 && <div className="h-1.5 rounded-full bg-natural-border"><div className="h-full rounded-full bg-natural-sage" style={{ width: `${progress}%` }} /></div>}</div></div>
    {path.length > 0 && <div><div className="mb-2"><h3 className="text-xs font-bold text-natural-dark">Chronological path</h3><p className="mt-1 text-sm text-natural-stone">Open a card to return to its saved reading session.</p></div><ol className="grid gap-2 md:grid-cols-2">{path.map((entry, index) => <li id={`ai-reader-path-${entry.log_id || index}`} key={entry.log_id || `${entry.page_start}-${index}`} className={returnTargetId === `ai-reader-path-${entry.log_id || index}` ? "rounded-2xl ring-2 ring-natural-sage/35" : undefined}><button type="button" onClick={() => entry.log_id && onSession(entry.log_id, `ai-reader-path-${entry.log_id || index}`)} disabled={!entry.log_id} className="h-full min-h-28 w-full rounded-2xl border border-natural-border bg-natural-cream/55 p-3 text-left transition hover:border-natural-sage/45 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-natural-sage/50 disabled:cursor-default"><div className="flex items-start justify-between gap-2"><span className="text-[10px] font-bold uppercase tracking-wider text-natural-sage">{String(index + 1).padStart(2, "0")}</span><span className="text-[10px] font-semibold text-natural-stone">{pageLabel(entry.page_start, entry.page_end, language, mode, labels)}</span></div><h4 className="mt-2 text-sm font-bold text-natural-dark">{entry.title}</h4><p className="mt-1 text-sm leading-relaxed text-natural-stone">{entry.turning_point || entry.summary}</p></button></li>)}</ol></div>}
...
function MapEvidenceGroup({ title, description, items, itemKey, language, mode, labels, openItem, setOpenItem, onSession, evidenceKey }: { title: string; description: string; items: Array<MapThread | MapEntity>; itemKey: string; language: GlossaryLanguage; mode: PageMode; labels?: Record<string, number | null>; openItem: string | null; setOpenItem: (value: string | null) => void; onSession: (id: string) => void; evidenceKey: "evolution" | "appearances" }) {
  return <div><h3 className="text-xs font-bold text-natural-dark">{title}</h3><p className="mt-1 text-sm text-natural-stone">{description}</p><div className="mt-2 space-y-2">{items.map((item, index) => {
    ...
    return <article key={key} className="overflow-hidden rounded-2xl bg-natural-cream/60">
      ...
      {state && <p className="px-3 pb-2 text-xs text-natural-sage">{stateTerm ? <GlossaryLabel term={stateTerm} language={language} /> : state}</p>}
      {open && <div className="space-y-2 border-t border-natural-border px-3 py-3">{evidence.length ? evidence.map((entry, evidenceIndex) => <button type="button" key={`${entry.log_id}-${evidenceIndex}`} onClick={() => entry.log_id && onSession(entry.log_id)} disabled={!entry.log_id} className="block w-full border-l-2 border-natural-sage/35 py-1 pl-2 text-left disabled:cursor-default">{pageLabel(entry.page_start, entry.page_start, language, mode, labels) ? <span className="text-xs font-bold text-natural-sage">{pageLabel(entry.page_start, entry.page_start, language, mode, labels)}</span> : null}<span className="ml-2 text-sm leading-relaxed text-natural-stone">{entry.note}</span></button>) : <p className="text-sm text-natural-stone">No path evidence is available yet.</p>}</div>}
    </article>;
  })}</div></div>;
}
...
function DrillGroup({ title, items, itemKey, language, openItem, setOpenItem }: { title: string; items: Record<string, unknown>[]; itemKey: string; language: GlossaryLanguage; openItem: string | null; setOpenItem: (value: string | null) => void }) {
  ...
  return <div><h3 className="mb-1 text-xs font-bold uppercase tracking-wider text-natural-sage">{title}</h3><div className="space-y-1">{items.map((item, index) => {
    const key = `${itemKey}-${item.id || index}`;
