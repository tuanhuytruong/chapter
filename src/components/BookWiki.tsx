...
function Person({ item }: { item: WikiPerson }) {
  const [open, setOpen] = useState(false);
  return <button onClick={() => setOpen(!open)} aria-expanded={open} className="w-full rounded-xl border border-natural-border bg-white/60 px-3 py-2 text-left transition hover:border-natural-sage/40 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-natural-sage"><span className="text-xs font-semibold text-natural-dark">{item.name}</span>{open && <p className="mt-1 text-[11px] leading-relaxed text-natural-stone">{item.pulse}</p>}</button>;
}
export default function BookWiki({ bookId, totalPages, canEdit }: { bookId: string; totalPages: number; canEdit: boolean }) {
  ...
  useEffect(() => { if (!running) return; const interval = window.setInterval(() => { void fetchWiki(); }, 7000); return () => clearInterval(interval); }, [running, fetchWiki]);
  const toggle = () => setExpanded(value => { localStorage.setItem(`book_wiki_expanded_${bookId}`, String(!value)); return !value; });
  const refresh = async () => { if (!canEdit || running) return; setRegenerating(true); try { await req(`/api/books/${bookId}/wiki/regenerate`, { method: "POST" }); await fetchWiki(); } finally { setRegenerating(false); } };
  ...
  if (!wiki) { const pct = Math.round((status.chunksProcessed / Math.max(status.totalSessions, 1)) * 100); return <div className="space-y-3">{heading}<div className="rounded-2xl border border-natural-border bg-natural-cream/40 p-5 text-center"><p className="text-xs font-semibold text-natural-dark">{running ? "AI Reader is running" : "AI Reader is preparing"}</p><p className="mt-1 text-[11px] text-natural-stone">{status.jobError || (status.chunksProcessed ? `${status.chunksProcessed} of ${status.totalSessions} sessions are ready.` : "It will be ready once the latest session is processed.")}</p><div className="mx-auto mt-3 h-1.5 max-w-xs rounded-full bg-natural-border"><div className="h-full rounded-full bg-natural-sage" style={{ width: `${pct}%` }}/></div></div>{canEdit && <button onClick={refresh} disabled={running || regenerating} className="min-h-11 w-full rounded-full border border-natural-sage/40 text-xs font-bold text-natural-sage disabled:opacity-50">{running || regenerating ? "Running…" : "Run AI Reader now"}</button>}</div>; }
