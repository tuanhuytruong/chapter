...
const sessionName = (item: ReaderSession, vi: boolean) => {
  const title = nameOf(item);
  if (title && title !== "Reading session" && title !== "Untitled") return title;
  if (item.page_start != null) return vi ? `Phiên đọc · Trang ${item.page_start}–${item.page_end ?? item.page_start}` : `Reading session · Pages ${item.page_start}–${item.page_end ?? item.page_start}`;
  return vi ? "Phiên đọc" : "Reading session";
};
const pageLabel = (start: number, end: number, vi: boolean) => vi ? `Trang ${start}–${end}` : `Pages ${start}–${end}`;
...
export default function BookWiki({ bookId, totalPages, canEdit }: { bookId: string; totalPages: number; canEdit: boolean }) {
  ...
  return <div className="space-y-4">
    ...
    {(wiki.reading_path?.length || wiki.thread_map?.length || wiki.entity_map?.length) ? <ReadingMap wiki={wiki} isVietnamese={isVietnamese} onSession={(id) => setOpenSession(id)} openItem={openItem} setOpenItem={setOpenItem} /> : null}
    <section><h2 className="mb-2 text-xs font-bold text-natural-dark">{isVietnamese ? "Các phiên đọc" : "Reading sessions"}</h2>{readerSessions.length ? <div className="space-y-2">{readerSessions.map((session, index) => { const key = session.id || `session-${index}`; const open = openSession === key; const entities = Array.isArray(session.entities) ? session.entities : []; const threads = Array.isArray(session.threads) ? session.threads : []; return <article key={key} className="overflow-hidden rounded-2xl border border-natural-border bg-white/55"><button onClick={() => setOpenSession(open ? null : key)} aria-expanded={open} className="flex min-h-12 w-full items-center justify-between gap-3 px-4 text-left"><span><b className="text-xs text-natural-dark">{sessionName(session, isVietnamese)}</b>{session.page_start != null && <small className="ml-2 text-[10px] text-natural-stone">{pageLabel(session.page_start, session.page_end ?? session.page_start, isVietnamese)}</small>}</span>{open ? <ChevronDown className="h-4 w-4 text-natural-sage" /> : <ChevronRight className="h-4 w-4 text-natural-stone" />}</button>{open && <div className="space-y-3 border-t border-natural-border px-4 py-3"><p className="text-[11px] leading-relaxed text-natural-stone">{text(session.summary) || (isVietnamese ? "Chi tiết phiên đọc đang được chuẩn bị." : "Session detail is not available yet.")}</p><DrillGroup title={isVietnamese ? "Nhân vật, ý tưởng & lực tác động" : "People & concepts"} items={entities} itemKey={`${key}-entity`} openItem={openItem} setOpenItem={setOpenItem} /><DrillGroup title={isVietnamese ? "Mạch truyện cần theo dõi" : "Threads to follow"} items={threads} itemKey={`${key}-thread`} openItem={openItem} setOpenItem={setOpenItem} /></div>}</article>; })}</div> : <p className="rounded-2xl border border-dashed border-natural-border p-4 text-center text-xs text-natural-stone">{isVietnamese ? "Chi tiết các phiên đọc đang được chuẩn bị." : "Session details are being prepared."}</p>}</section>
    {!sessions && <V1References wiki={wiki} />}
  </div>;
}
function ReadingMap({ wiki, isVietnamese, onSession, openItem, setOpenItem }: { wiki: BookWikiData; isVietnamese: boolean; onSession: (id: string) => void; openItem: string | null; setOpenItem: (value: string | null) => void }) {
  const threads = (wiki.thread_map || []).map((item) => ({ ...item, detail: item.evolution?.map((e) => `${pageLabel(e.page_start, e.page_start, isVietnamese)}: ${e.note}`).join(" · ") }));
  const entities = (wiki.entity_map || []).map((item) => ({ ...item, detail: [item.current_state, item.appearances?.map((e) => `${pageLabel(e.page_start, e.page_start, isVietnamese)}: ${e.note}`).join(" · ")].filter(Boolean).join(" ") }));
  return <section className="space-y-3 rounded-2xl border border-natural-border bg-white/45 p-4">
    <div><h2 className="text-xs font-bold text-natural-dark">{isVietnamese ? "Bản đồ mạch đọc" : "Reading map"}</h2><p className="mt-1 text-[11px] text-natural-stone">{isVietnamese ? "Theo dõi hành trình, rồi mở từng mạch hoặc nhân tố khi chúng thay đổi." : "Follow the path, then open each thread or force where it changes."}</p></div>
    {wiki.reading_path?.length ? <ol className="space-y-2">{wiki.reading_path.map((entry) => <li key={entry.log_id}><button onClick={() => onSession(entry.log_id)} className="w-full border-l-2 border-natural-sage/40 py-1 pl-3 text-left"><span className="text-xs font-semibold text-natural-dark">{entry.title} <small className="ml-1 text-[10px] font-bold text-natural-stone">{pageLabel(entry.page_start, entry.page_end, isVietnamese)}</small></span><p className="mt-0.5 text-[11px] leading-relaxed text-natural-stone">{entry.turning_point || entry.summary}</p></button></li>)}</ol> : null}
    <DrillGroup title={isVietnamese ? "Mạch truyện đang tiếp diễn" : "Threads in motion"} items={threads} itemKey="thread-map" openItem={openItem} setOpenItem={setOpenItem} />
    <DrillGroup title={isVietnamese ? "Nhân vật, ý tưởng & lực tác động" : "People, ideas & forces"} items={entities} itemKey="entity-map" openItem={openItem} setOpenItem={setOpenItem} />
  </section>;
}
