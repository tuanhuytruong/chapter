...
const pageLabel = (start: number, end: number, language: GlossaryLanguage = "en") => `${language === "vi" ? "Trang" : "Pages"} ${start}–${end}`;
...
export default function BookWiki({ bookId, totalPages, canEdit }: { bookId: string; totalPages: number; canEdit: boolean }) {
  ...
  return <div className="space-y-4">
    ...
    {(wiki.reading_path?.length || wiki.thread_map?.length || wiki.entity_map?.length || wiki.connections?.length) ? <ReadingMap wiki={wiki} totalPages={totalPages} status={status?.status || status?.jobStatus} language={glossaryLanguage} onSession={(id) => preserveScroll(() => setOpenSession(id))} openItem={openItem} setOpenItem={(value) => preserveScroll(() => setOpenItem(value))} /> : null}
    <section><h2 className="mb-2 text-xs font-bold text-natural-dark">Reading sessions</h2>{readerSessions.length ? <div className="space-y-2">{readerSessions.map((session, index) => { const key = session.id || `session-${index}`; const open = openSession === key; const entities = Array.isArray(session.entities) ? session.entities : []; const threads = Array.isArray(session.threads) ? session.threads : []; return <article key={key} className="overflow-hidden rounded-2xl border border-natural-border bg-white/55"><button type="button" onClick={() => preserveScroll(() => setOpenSession(open ? null : key))} aria-expanded={open} className="flex min-h-12 w-full items-center justify-between gap-3 px-4 text-left"><span><b className="text-xs text-natural-dark">{sessionName(session)}</b>{session.page_start != null && <small className="ml-2 text-[10px] text-natural-stone">{pageLabel(session.page_start, session.page_end ?? session.page_start, glossaryLanguage)}</small>}</span>{open ? <ChevronDown className="h-4 w-4 text-natural-sage" /> : <ChevronRight className="h-4 w-4 text-natural-stone" />}</button>{open && <div className="space-y-3 border-t border-natural-border px-4 py-3"><p className="text-[11px] leading-relaxed text-natural-stone">{text(session.summary) || "Session detail is not available yet."}</p><DrillGroup title="People & concepts" items={entities} itemKey={`${key}-entity`} language={glossaryLanguage} openItem={openItem} setOpenItem={(value) => preserveScroll(() => setOpenItem(value))} /><DrillGroup title="Threads to follow" items={threads} itemKey={`${key}-thread`} language={glossaryLanguage} openItem={openItem} setOpenItem={(value) => preserveScroll(() => setOpenItem(value))} /></div>}</article>; })}</div> : <p className="rounded-2xl border border-dashed border-natural-border p-4 text-center text-xs text-natural-stone">Session details are being prepared.</p>}</section>
    {!sessions && <V1References wiki={wiki} language={glossaryLanguage} preserveScroll={preserveScroll} />}
  </div>;
}
function ReadingMap({ wiki, totalPages, status, language, onSession, openItem, setOpenItem }: { wiki: BookWikiData; totalPages: number; status?: string; language: GlossaryLanguage; onSession: (id: string) => void; openItem: string | null; setOpenItem: (value: string | null) => void }) {
