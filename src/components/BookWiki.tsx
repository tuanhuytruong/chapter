...
const nameOf = (item: Record<string, unknown>) => text(item.name) || text(item.title) || text(item.label) || "Untitled";
...
export default function BookWiki({ bookId, totalPages, canEdit }: { bookId: string; totalPages: number; canEdit: boolean }) {
  ...
  return <div className="space-y-4">
    ...
    {(wiki.reading_path?.length || wiki.thread_map?.length || wiki.entity_map?.length) ? <ReadingMap wiki={wiki} onSession={(id) => setOpenSession(id)} openItem={openItem} setOpenItem={setOpenItem} /> : null}
    <section><h2 className="mb-2 text-xs font-bold text-natural-dark">Reading sessions</h2>{readerSessions.length ? <div className="space-y-2">{readerSessions.map((session, index) => { const key = session.id || `session-${index}`; const open = openSession === key; const entities = Array.isArray(session.entities) ? session.entities : []; const threads = Array.isArray(session.threads) ? session.threads : []; return <article key={key} className="overflow-hidden rounded-2xl border border-natural-border bg-white/55"><button onClick={() => setOpenSession(open ? null : key)} aria-expanded={open} className="flex min-h-12 w-full items-center justify-between gap-3 px-4 text-left"><span><b className="text-xs text-natural-dark">{nameOf(session)}</b>{session.page_start != null && <small className="ml-2 text-[10px] text-natural-stone">pp. {session.page_start}–{session.page_end ?? session.page_start}</small>}</span>{open ? <ChevronDown className="h-4 w-4 text-natural-sage" /> : <ChevronRight className="h-4 w-4 text-natural-stone" />}</button>{open && <div className="space-y-3 border-t border-natural-border px-4 py-3"><p className="text-[11px] leading-relaxed text-natural-stone">{text(session.summary) || "Session detail is not available yet."}</p><DrillGroup title="People & concepts" items={entities} itemKey={`${key}-entity`} openItem={openItem} setOpenItem={setOpenItem} /><DrillGroup title="Threads to follow" items={threads} itemKey={`${key}-thread`} openItem={openItem} setOpenItem={setOpenItem} /></div>}</article>; })}</div> : <p className="rounded-2xl border border-dashed border-natural-border p-4 text-center text-xs text-natural-stone">Session details are being prepared.</p>}</section>
...
function ReadingMap({ wiki, onSession, openItem, setOpenItem }: { wiki: BookWikiData; onSession: (id: string) => void; openItem: string | null; setOpenItem: (value: string | null) => void }) {
  return <section className="space-y-3 rounded-2xl border border-natural-border bg-white/45 p-4">
    ...
    {wiki.reading_path?.length ? <ol className="space-y-2">{wiki.reading_path.map((entry) => <li key={entry.log_id}><button onClick={() => onSession(entry.log_id)} className="w-full border-l-2 border-natural-sage/40 py-1 pl-3 text-left"><span className="text-xs font-semibold text-natural-dark">{entry.title} <small className="font-normal text-natural-stone">pp. {entry.page_start}–{entry.page_end}</small></span><p className="mt-0.5 text-[11px] leading-relaxed text-natural-stone">{entry.turning_point || entry.summary}</p></button></li>)}</ol> : null}
    <DrillGroup title="Threads in motion" items={(wiki.thread_map || []).map((item) => ({ ...item, detail: item.evolution?.map((e) => `p.${e.page_start}: ${e.note}`).join(" · ") }))} itemKey="thread-map" openItem={openItem} setOpenItem={setOpenItem} />
    <DrillGroup title="People, ideas & forces" items={(wiki.entity_map || []).map((item) => ({ ...item, detail: [item.current_state, item.appearances?.map((e) => `p.${e.page_start}: ${e.note}`).join(" · ")].filter(Boolean).join(" ") }))} itemKey="entity-map" openItem={openItem} setOpenItem={setOpenItem} />
  </section>;
}
function DrillGroup({ title, items, itemKey, openItem, setOpenItem }: { title: string; items: Record<string, unknown>[]; itemKey: string; openItem: string | null; setOpenItem: (value: string | null) => void }) { if (!items.length) return null; return <div><h3 className="mb-1 text-[10px] font-bold uppercase tracking-wider text-natural-sage">{title}</h3><div className="space-y-1">{items.map((item, index) => { const key = `${itemKey}-${item.id || index}`; const detail = text(item.description) || text(item.detail) || text(item.summary); return <button key={key} onClick={() => setOpenItem(openItem === key ? null : key)} className="w-full rounded-xl bg-natural-cream/60 px-3 py-2 text-left"><span className="text-xs font-semibold text-natural-dark">{nameOf(item)}</span>{openItem === key && detail && <p className="mt-1 text-[11px] leading-relaxed text-natural-stone">{detail}</p>}</button>; })}</div></div>; }
