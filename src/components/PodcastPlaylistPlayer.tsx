...
export default function PodcastPlaylistPlayer({ bookId, compact = false }: { bookId: string; compact?: boolean }) {
  ...
  useEffect(() => {
    ...
    const restore = () => { if (resumeAt > 1 && Math.abs(audio.currentTime - resumeAt) > 1) audio.currentTime = resumeAt; };
  ...
  return <section className={`mt-4 rounded-2xl border border-natural-sage/25 bg-natural-sage/5 p-3 ${compact ? "" : "sm:p-4"}`} aria-label="Book podcast playlist">
    ...
    {active && <div className="mt-3 rounded-xl bg-white/75 p-3"><p className="text-xs font-bold text-natural-dark">Now playing · {episodeName(active, activeIndex || 0)}</p><audio ref={audioRef} className="mt-2 w-full" controls preload="metadata" src={`/api/podcasts/${active.id}/audio`} onPlay={() => setPlaying(true)} onPause={() => { setPlaying(false); save(false); }} onTimeUpdate={(event) => { const now = Date.now(); if (now - lastSavedAt.current > 15000) { lastSavedAt.current = now; save(false, event.currentTarget.currentTime); } }} onEnded={() => { save(true); setPlaying(false); if (started && activeIndex !== null && activeIndex + 1 < episodes.length) { setActiveIndex(activeIndex + 1); window.setTimeout(() => void audioRef.current?.play().catch(() => undefined), 100); } }} onError={() => setPlaying(false)} />
