  ...
  return <main className="mx-auto max-w-4xl space-y-5 font-sans">
    ...
    {loading && !books.length ? <div className="flex justify-center py-12"><Loader2 className="animate-spin text-natural-sage" /></div> : books.length ? books.map((book) => {
      ...
      return <section key={book.id} className="overflow-hidden rounded-[24px] border border-natural-border bg-natural-cream shadow-sm">
        ...
        {open && <div className="border-t border-natural-border px-4 py-4 sm:px-5"><PodcastPlaylistPlayer key={book.id} bookId={book.id} playRequest={playRequest && playRequest.bookId === book.id ? playRequest : null} onPlayed={() => setPlayRequest(null)} />{book.chapters.map((chapter) => <div key={chapter.chapter_key}><ChapterRow chapter={chapter} creating={creatingKey === `${book.id}:${chapter.chapter_key}`} onCreate={() => { void create(book.id, chapter.chapter_key); }} onPlay={() => { const episode = chapter.episode as PodcastEpisode | null; if (episode) setPlayRequest({ bookId: book.id, episodeId: episode.id }); }} /></div>)}</div>}
      </section>;
...
function ChapterRow({ chapter, creating, onCreate, onPlay }: { chapter: PodcastChapter; creating: boolean; onCreate: () => void; onPlay: () => void }) {
  ...
  return <article className="border-b border-natural-border/80 py-4 last:border-b-0"><div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div className="min-w-0"><p className="text-sm font-bold text-natural-dark">{chapter.chapter_title || "Untitled chapter"}</p><p className="mt-0.5 text-[11px] text-natural-stone">{chapter.start_page != null ? `Pages ${chapter.start_page}${chapter.end_page != null && chapter.end_page !== chapter.start_page ? `–${chapter.end_page}` : ""}` : `Sections ${chapter.start_unit}–${chapter.end_unit}`}{locallyReady && duration(episode?.duration_s || null) ? ` · ${duration(episode?.duration_s || null)}` : ""}</p></div>{locallyReady ? <button onClick={onPlay} className="min-h-11 shrink-0 rounded-full bg-natural-sage px-4 text-xs font-bold text-white"><Play className="mr-1 inline h-3.5 w-3.5" /> Play</button> : episode?.status === "unavailable" ? null : <button onClick={onCreate} disabled={creating || running} className="min-h-11 shrink-0 rounded-full border border-natural-border px-4 text-xs font-bold text-natural-dark disabled:opacity-60">{creating || running ? "Creating…" : episode?.status === "failed" ? "Try again" : "Create episode"}</button>}</div>{running && <p className="mt-2 text-xs text-natural-stone">episode?.status === "synthesizing" ? "Audio service is preparing this episode. It will retry automatically if needed." : "Preparing this chapter episode…"</p>}{episode?.status === "archive_pending" && <p className="mt-2 text-xs text-natural-sage">Ready to listen. Private archive retry is continuing in the background.</p>}{episode?.status === "failed" && <p className="mt-2 text-xs text-natural-clay">This episode could not be prepared. You can try again.</p>}{episode?.status === "unavailable" && <p className="mt-2 text-xs text-natural-stone"><span className="font-semibold text-natural-dark">Episode unavailable.</span> This section is too brief for a meaningful podcast{episode.word_count != null ? ` (${episode.word_count} words)` : ""}. The next eligible chapter will be prepared instead.</p>}{locallyReady && <details className="mt-3"><summary className="cursor-pointer text-xs font-semibold text-natural-stone">Read transcript</summary><p className="mt-2 whitespace-pre-wrap text-sm leading-6 text-natural-dark">{episode?.script_text}</p></details>}</article>;
}
