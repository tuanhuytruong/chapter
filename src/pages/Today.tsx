...
export default function Today() {
  ...
  const respondToTodayReturn = async (outcome: ReturnOutcome, reflection?: string) => {
    ...
    try {
      ...
      const revisit = returnCard;
      setReturnCard(null);
      if (outcome === "revisit") {
        const params = new URLSearchParams({ returnLog: revisit.log_id });
        if (revisit.source_reading_round != null) params.set("returnRound", String(revisit.source_reading_round));
        navigate(`/books/${revisit.book_id}?${params.toString()}`);
      }
    } catch (cause: any) {
  ...
  useEffect(() => {
    ...
    void api.getReturns("today", active.id)
      .then((cards) => {
        if (!mounted) return;
  ...
  const { next_queued_book: next, today_progress: today } = dashboard;
  ...
  return <main className="mx-auto max-w-5xl space-y-5 px-4 font-sans sm:px-0">
    ...
    {active ? <section aria-labelledby="current-reads-heading" className="rounded-[28px] border border-natural-border bg-natural-cream p-4 shadow-sm sm:p-5">
      ...
      {activeBooks.length > 1 && <div role="list" aria-label="Active books" className="mb-4 flex flex-wrap gap-2">{activeBooks.map((book) => <button key={book.id} type="button" onClick={() => chooseActiveBook(book.id)} aria-pressed={book.id === active.id} className={`flex min-h-11 max-w-full items-center gap-2 rounded-xl border px-2 py-1 text-left text-xs font-bold focus:outline-none focus:ring-2 focus:ring-natural-sage ${book.id === active.id ? "today-tab-active" : "today-tab-idle"}`}><span className="flex h-8 w-6 shrink-0 items-center justify-center overflow-hidden rounded bg-natural-sage/15 text-natural-sage">{book.cover_url ? <img src={book.cover_url} alt="" className="h-full w-full object-cover" /> : <BookOpen className="h-3.5 w-3.5" />}</span><span className="max-w-40 truncate">{book.title}</span><span className="shrink-0 font-normal">{progressPct(book)}%</span></button>)}</div>}
      <div className="grid gap-4 lg:grid-cols-[minmax(0,1.15fr)_minmax(18rem,0.85fr)]">
        <article className="today-card-surface rounded-2xl border p-4 sm:p-5"><p className="text-xs font-bold uppercase tracking-wider text-natural-sage">Continue reading</p><div className="mt-4 flex gap-4"><div className="flex h-20 w-14 shrink-0 items-center justify-center overflow-hidden rounded-lg bg-natural-sage/15 text-natural-sage">{active.cover_url ? <img src={active.cover_url} alt="" fetchPriority="high" className="h-full w-full object-cover" /> : <BookOpen className="h-6 w-6" />}</div><div className="min-w-0 flex-1"><h3 className="truncate text-lg font-bold text-natural-dark">{active.title}</h3><p className="truncate text-sm text-natural-stone">{active.author}</p><div className="mt-3 today-progress-track h-2 overflow-hidden rounded-full"><div className="h-full rounded-full bg-natural-sage" style={{ width: `${progressPct(active)}%` }} /></div><p className="mt-1 text-xs text-natural-stone">{active.current_page} / {active.total_pages} {active.file_type === "epub" ? "chunks" : "pages"}</p></div></div><Link to={`/books/${active.id}`} className="mt-5 flex min-h-11 w-full items-center justify-center gap-2 rounded-xl bg-natural-sage px-4 text-sm font-bold text-white hover:opacity-90"><Play className="h-4 w-4" /> Read next session</Link></article>
        <article aria-labelledby="today-insights-heading" className="today-card-surface rounded-2xl border p-4 sm:p-5"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><p className="text-xs font-bold uppercase tracking-wider text-natural-sage">Key insights so far</p><h3 id="today-insights-heading" className="mt-1 truncate font-bold text-natural-dark">{active.title}</h3><p className="mt-1 text-xs text-natural-stone">From your current read</p></div><div className="flex shrink-0 flex-col items-end text-sm font-bold text-natural-sage"><Link to="/insights" className="inline-flex min-h-8 items-center gap-1">More insights <ArrowRight className="h-4 w-4" /></Link><Link to={`/books/${active.id}`} className="inline-flex min-h-8 items-center gap-1">Open book <ArrowRight className="h-4 w-4" /></Link></div></div>{insightsLoading ? <p className="mt-5 flex items-center gap-2 text-sm text-natural-stone"><Loader2 className="h-4 w-4 animate-spin" /> Gathering your ideas…</p> : insightsError ? <p role="alert" className="mt-5 text-sm text-natural-stone">{insightsError}</p> : insights?.insights.length ? <ul className="mt-4 space-y-2">{insights.insights.slice(0, 3).map((insight, index) => <li key={insight.text} className="flex items-start gap-2 today-insight-row rounded-xl px-3 py-2 text-sm"><span className="mt-0.5 flex h-5 w-5 shrink-0 items-center justify-center rounded-full today-insight-index border text-[9px] font-bold">{index + 1}</span><p className="min-w-0 flex-1 leading-relaxed"><InlineMarkdown text={stripInsightOrdinal(insight.text)} />{insight.occurrences > 1 && <span className="ml-1 today-insight-count whitespace-nowrap text-xs">{insight.occurrences} times</span>}</p></li>)}</ul> : <p className="mt-5 text-sm text-natural-stone">Your insights will gather here after a reading session.</p>}</article>
      </div>
    ...
    {returnCard && <section aria-label="Today return" className="rounded-[28px] border border-natural-border bg-white p-4 shadow-sm sm:p-5"><div className="mb-4 flex items-center justify-between gap-3"><div><p className="text-xs font-bold uppercase tracking-wider text-natural-sage">Today’s return</p><p className="mt-1 text-sm text-natural-stone">From the book you are reading now.</p></div><Link to="/review" className="shrink-0 text-xs font-bold text-natural-sage hover:underline">All returns</Link></div><ReturnCard card={returnCard} saving={returnSaving} onRespond={(outcome, reflection) => void respondToTodayReturn(outcome, reflection)} onSourceOpen={() => captureAnalyticsEvent("return_source_opened", { surface: "today" })} />{returnError && <p role="alert" className="mt-3 text-center text-xs text-red-700">{returnError}</p>}</section>}

    <section className="today-status-surface flex flex-wrap items-center gap-3 rounded-2xl border p-4 text-sm"><CheckCircle2 className="h-5 w-5 text-natural-sage" /><p className="text-natural-dark"><span className="font-bold">Today:</span> {today.sessions} reading session{today.sessions === 1 ? "" : "s"} · {today.units} pages / chunks</p></section>
  </main>;
}
