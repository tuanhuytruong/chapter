...
export default function BookDetail() {
  ...
  useEffect(() => {
    ...
    if (!target || !book?.can_edit || book.status !== 'active') {
  ...
  return (
    <div className="space-y-6 font-sans">
      ...
      {book.can_edit && book.status === 'active' && !headerReadActionVisible && (
        <div className="fixed inset-x-3 bottom-[calc(env(safe-area-inset-bottom)+1rem)] z-40 pointer-events-none sm:inset-x-auto sm:bottom-20 sm:right-6">
          <button onClick={readToday} disabled={advancing}
            aria-label="Read next session"
            className="pointer-events-auto flex min-h-11 w-full items-center justify-center gap-1.5 rounded-full bg-natural-clay px-5 py-2.5 text-xs font-bold uppercase tracking-wider text-white shadow-lg shadow-natural-dark/20 hover:opacity-90 disabled:opacity-50 cursor-pointer sm:w-auto">
