...
function BadgeCard({ tier, earned }: { tier: QuietStreakTier; earned: boolean }) {
  return <article className={`rounded-2xl border p-4 ${earned ? "border-natural-sage/35 bg-natural-sage/10" : "border-natural-border bg-natural-cream"}`}>
    <div className="flex items-start gap-3"><div className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-full ${earned ? "bg-natural-sage text-white" : "bg-natural-bg text-natural-stone"}`}><Award className="h-5 w-5" /></div><div className="min-w-0"><div className="flex items-center gap-2"><h2 className="font-bold text-natural-dark">{tier.title}</h2>{earned && <CheckCircle2 aria-label="Earned" className="h-4 w-4 text-natural-sage" />}</div><p className="mt-1 text-xs leading-relaxed text-natural-stone">{tier.description}</p><p className="mt-3 text-xs font-bold uppercase tracking-wider text-natural-stone">{tier.days} days · {earned ? "Earned" : "Ahead"}</p></div></div>
  </article>;
}
