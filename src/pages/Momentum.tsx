...
export default function Momentum() {
  ...
  return <main className="mx-auto max-w-3xl space-y-5 font-sans">
    ...
    <form onSubmit={save} className="rounded-[28px] border border-natural-border bg-natural-cream p-5 shadow-sm sm:p-6"><h2 className="font-bold text-natural-dark">{goal ? "Adjust weekly goal" : "Your weekly goal"}</h2><div className="mt-4 grid gap-3 sm:grid-cols-[1fr_140px_auto]"><label className="text-sm font-medium text-natural-dark">Measure<select value={metric} onChange={(event) => setMetric(event.target.value as WeeklyGoalMetric)} className="mt-1 min-h-11 w-full rounded-xl border border-natural-border bg-white px-3 text-sm"><option value="sessions">Reading sessions</option><option value="units">Pages / chunks</option></select></label><label className="text-sm font-medium text-natural-dark">Target<input value={target} onChange={(event) => setTarget(event.target.value)} inputMode="numeric" type="number" min="1" max="10000" className="mt-1 min-h-11 w-full rounded-xl border border-natural-border bg-white px-3 text-sm" /></label><button disabled={saving} className="mt-5 flex min-h-11 items-center justify-center gap-2 rounded-xl bg-natural-sage px-4 text-sm font-bold text-white disabled:opacity-60"><Save className="h-4 w-4" />{saving ? "Saving…" : goal ? "Save changes" : "Set goal"}</button></div></form>
