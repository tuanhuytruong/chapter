...
export default function Library() {
  ...
  return (
    <div className="space-y-6">
      {scope === 'mine' && !loading && books.length === 0 && <GuideCard step="welcome" title="Begin with one book, one small session"><p>Chapter keeps the reading surface quiet: add a book, read at your pace, and let the companion notes arrive after your progress is safely saved.</p><button onClick={() => { void dismiss('welcome'); captureAnalyticsEvent("book_add_started", { source: "welcome" }); setShowAdd(true); }} className="min-h-11 rounded-full bg-natural-sage px-4 text-xs font-bold text-white hover:bg-natural-sage-dark">Add your first book</button></GuideCard>}
      ...
      <div className="flex flex-col items-stretch justify-between gap-3 sm:flex-row sm:items-center">
        ...
        {scope === 'mine' && <button onClick={() => { captureAnalyticsEvent("book_add_started", { source: "library" }); setShowAdd(true); }} className="flex min-h-11 items-center justify-center gap-1.5 self-start rounded-full bg-natural-sage px-4 py-2.5 font-sans text-xs font-bold uppercase tracking-wider text-white shadow-sm hover:bg-natural-sage-dark sm:self-auto"><Plus className="h-3.5 w-3.5" /> Add Book</button>}
      </div>
      ...
      {scope === 'mine' && globalQuery.length >= 2 ? <LibrarySearchResults results={globalResults} loading={globalLoading} error={globalError} onOpen={openSearchResult} /> : loading ? <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">{[0, 1, 2, 3, 4, 5].map((index) => <div key={index} className="h-36 animate-pulse rounded-[24px] border border-natural-border bg-natural-cream" />)}</div>
        : showQueue && queued.length > 0 ? <section className="rounded-[28px] border border-natural-border bg-natural-cream p-4 shadow-xs sm:p-6">
          ...
          : visible.length === 0 ? <div className="flex flex-col items-center justify-center space-y-3 rounded-[32px] border border-natural-border bg-natural-cream p-16 text-center"><BookOpen className="h-10 w-10 text-natural-stone" /><p className="font-sans text-sm font-bold text-natural-dark">{showQueue ? 'Your queue is clear' : 'No books here yet'}</p><p className="font-sans text-xs text-natural-stone">{emptyCopy}</p>{scope === 'mine' && <button onClick={() => { captureAnalyticsEvent("book_add_started", { source: "library" }); setShowAdd(true); }} className="flex items-center gap-1.5 rounded-full bg-natural-sage px-4 py-2.5 font-sans text-xs font-bold uppercase tracking-wider text-white"><Plus className="h-3.5 w-3.5" /> Add Book</button>}</div>
            : scope === 'all' ? <div className="space-y-6">{readerGroups.map((group) => <section key={group.name} className="space-y-3"><h2 className="font-sans text-sm font-bold text-natural-dark">{group.name}'s shelf <span className="font-normal text-natural-stone">({group.books.length})</span></h2><div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">{group.books.map((book, index) => <BookCard key={book.id} book={book} streak={streaks[book.id]} readOnly lazyCover={index >= 6} />)}</div></section>)}</div>
              : <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">{visible.map((book, index) => <BookCard key={book.id} book={book} streak={streaks[book.id]} lazyCover={index >= 6} />)}</div>}
