...
export default function Quotes() {
  ...
  return <main className="mx-auto max-w-3xl space-y-6"><div className="flex flex-wrap items-end justify-between gap-3"><div><Link to="/" className="flex min-h-10 items-center gap-1.5 font-sans text-xs font-bold text-natural-stone hover:text-natural-dark"><ArrowLeft className="h-3.5 w-3.5" /> Back to my shelf</Link><p className="mt-4 font-sans text-xs font-bold uppercase tracking-[0.16em] text-natural-sage">Your reading archive</p><h1 className="mt-1 font-sans text-3xl italic text-natural-dark">All saved lines</h1><p className="mt-1 font-sans text-xs text-natural-stone">{total ? `${total} line${total === 1 ? '' : 's'} worth keeping` : 'A quiet place for what stayed with you'}</p></div></div>
