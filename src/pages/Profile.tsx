...
export default function Profile() {
  ...
  return <main className="mx-auto max-w-2xl space-y-5">
    ...
    <form onSubmit={save} className="space-y-6 rounded-3xl border border-natural-border bg-natural-cream p-5 shadow-sm sm:p-6">
      <div className="flex items-center gap-4"><div className={`flex h-16 w-16 items-center justify-center rounded-[22px] ${current.tone} text-3xl shadow-sm`} aria-label={`${current.label} avatar`}>{current.emoji}</div><div><p className="font-sans text-sm font-bold text-natural-dark">{displayName || "Your profile"}</p><p className="mt-1 font-mono text-[10px] uppercase tracking-wider text-natural-stone">@{user?.username}</p></div></div>
      ...
      <fieldset><legend className="font-sans text-xs font-bold text-natural-dark">Choose an animal companion</legend><div className="mt-3 grid grid-cols-4 gap-2 sm:grid-cols-8">{AVATAR_PRESETS.map((item) => <button key={item.id} type="button" onClick={() => setAvatar(item.id)} aria-label={`Choose ${item.label}`} aria-pressed={item.id === avatar} className={`relative flex min-h-16 flex-col items-center justify-center rounded-2xl border text-2xl transition ${item.id === avatar ? "border-natural-sage bg-natural-sage/10 ring-2 ring-natural-sage/25" : "border-natural-border bg-white hover:border-natural-sage/45"}`}><span>{item.emoji}</span><span className="mt-1 font-sans text-[9px] font-bold text-natural-stone">{item.label}</span>{item.id === avatar && <Check className="absolute right-1.5 top-1.5 h-3.5 w-3.5 text-natural-sage" />}</button>)}</div></fieldset>
