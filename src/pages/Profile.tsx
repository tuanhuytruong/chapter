...
import { AVATAR_PRESETS, avatarSrcForPreset, avatarValueForPreset, presetFromAvatarValue, type AvatarPresetId } from "../avatar-presets";
...
export default function Profile() {
  ...
  return <main className="mx-auto max-w-2xl space-y-5">
    ...
    <form onSubmit={save} className="space-y-6 rounded-3xl border border-natural-border bg-natural-cream p-5 shadow-sm sm:p-6">
      <div className="flex items-center gap-4"><div className={`flex h-16 w-16 overflow-hidden rounded-[22px] ${current.tone} shadow-sm`}><img src={avatarSrcForPreset(current.id, 128)} alt={`${current.label} geometric mascot`} className="h-full w-full object-cover" /></div><div><p className="font-sans text-sm font-bold text-natural-dark">{displayName || "Your profile"}</p><p className="mt-1 font-mono text-[10px] uppercase tracking-wider text-natural-stone">@{user?.username}</p></div></div>
