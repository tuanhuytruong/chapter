...
-- Authenticated reader feedback. Store only a bounded page path and broad client category.
...
-- Rate limiter key is a one-way owner-derived hash; it never stores reader identity or content.
...
-- Listening rhythm (twin-track): one row per (user, episode, day), idempotent
-- when an episode is re-listened in a day. Listened_on is the Asia/Bangkok day.
...
-- Returns response history is append-only. Production uses the matching manual
-- migration before response inserts; this declaration supports clean installs.
...
-- chapter.reading_progress_companions
CREATE TABLE IF NOT EXISTS chapter.reading_progress_companions (
 book_id UUID NOT NULL REFERENCES chapter.books(id) ON DELETE CASCADE, reading_round INT NOT NULL CHECK(reading_round>=1), schema_version SMALLINT NOT NULL DEFAULT 1, main_thread JSONB NOT NULL DEFAULT '{}'::jsonb, converging JSONB NOT NULL DEFAULT '[]'::jsonb, open_threads JSONB NOT NULL DEFAULT '[]'::jsonb, carry_forward JSONB NOT NULL DEFAULT '[]'::jsonb, output_language TEXT NOT NULL CHECK(output_language IN ('vi','en')), sessions_covered INT NOT NULL DEFAULT 0, last_log_id UUID REFERENCES chapter.reading_log(id), last_log_date DATE, last_log_session INT, source_revision BIGINT NOT NULL DEFAULT 1, stale BOOLEAN NOT NULL DEFAULT FALSE, generated_at TIMESTAMPTZ NOT NULL DEFAULT now(), PRIMARY KEY(book_id,reading_round)
);
CREATE INDEX IF NOT EXISTS idx_reading_progress_companions_book_round ON chapter.reading_progress_companions(book_id,reading_round);



-- chapter.reading_progress_companion_facts
CREATE TABLE IF NOT EXISTS chapter.reading_progress_companion_facts (
 book_id UUID NOT NULL REFERENCES chapter.books(id) ON DELETE CASCADE, reading_round INT NOT NULL CHECK(reading_round>=1), log_id UUID NOT NULL REFERENCES chapter.reading_log(id) ON DELETE CASCADE, source_hash TEXT NOT NULL, facts JSONB NOT NULL DEFAULT '[]'::jsonb, output_language TEXT NOT NULL CHECK(output_language IN ('vi','en')), created_at TIMESTAMPTZ NOT NULL DEFAULT now(), generated_at TIMESTAMPTZ NOT NULL DEFAULT now(), PRIMARY KEY(book_id,reading_round,log_id)
);
CREATE INDEX IF NOT EXISTS idx_reading_progress_companion_facts_round ON chapter.reading_progress_companion_facts(book_id,reading_round,created_at);
...
-- Reference books retain the ordinary reading-log pipeline but produce no
-- analytical or Story Thread enrichment. Their reusable cards are private to
-- the owner and preserve an immutable source pointer into a saved log.
ALTER TABLE chapter.books DROP CONSTRAINT IF EXISTS books_reading_experience_check;
ALTER TABLE chapter.books ADD CONSTRAINT books_reading_experience_check
