...
export function useJobPolling<T>({ enabled, intervalMs, poll, onSuccess, onError, maxBackoffMs = 30_000 }: JobPollingOptions<T>) {
  const options = useRef({ enabled, intervalMs, poll, onSuccess, onError, maxBackoffMs });
  options.current = { enabled, intervalMs, poll, onSuccess, onError, maxBackoffMs };
  ...
  useEffect(() => {
    ...
    const schedule = (delay: number) => {
      clear();
      if (!disposed && options.current.enabled && !document.hidden) timer = window.setTimeout(run, delay);
    };
    const run = async () => {
      if (disposed || inFlight || document.hidden || !options.current.enabled) return;
      inFlight = true;
