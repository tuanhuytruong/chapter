      ...
      <header className="sticky top-0 z-40 border-b border-natural-border bg-natural-bg">
        <div className="mx-auto flex min-h-16 w-full max-w-7xl items-center justify-between gap-2 px-3 sm:h-20 sm:px-6 lg:px-8">
          ...
          <div className="flex shrink-0 items-center gap-1 sm:gap-2">
            ...
            <NavLink to="/profile" aria-label="Your profile" title="Your profile" className="flex min-h-10 items-center gap-1.5 rounded-full outline-none hover:opacity-70 focus-visible:ring-2 focus-visible:ring-natural-sage/50 focus-visible:ring-offset-2 focus-visible:ring-offset-natural-bg"><div className={`flex h-8 w-8 overflow-hidden rounded-full ${presetFromAvatarValue(user?.avatarUrl)?.tone || 'bg-natural-sage/20'}`}>{presetFromAvatarValue(user?.avatarUrl) ? <AnimalAvatar id={presetFromAvatarValue(user?.avatarUrl)!.id} className="h-full w-full" /> : <span className="flex h-full w-full items-center justify-center font-sans text-[10px] font-bold text-natural-sage">{user?.displayName?.[0]?.toUpperCase()}</span>}</div><span className="hidden max-w-[100px] truncate font-sans text-xs font-medium text-natural-dark lg:inline">{user?.displayName}</span></NavLink>
