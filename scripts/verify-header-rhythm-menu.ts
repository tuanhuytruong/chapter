...
assert.match(shell, /aria-haspopup="menu"/); assert.match(shell, /desktop-account-menu/); assert.match(shell, /ReadingRhythmChip/); assert.match(shell, /to="\/streaks" onClick=\{\(\) => closeAccountMenu\(\)\}/); assert.match(shell, /Plan & membership/); assert.match(shell, /onPointerDown/); assert.match(shell, /event.key === 'Escape'/); assert.doesNotMatch(shell, /<MembershipTierBadge/); assert.doesNotMatch(shell, /QuietStreakBadge/);
