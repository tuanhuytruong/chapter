...
assert.match(readToday, /setLogs\(\(previous\) =>\s*sortLogsNewestFirst\(\[\s*result\.log,\s*\.\.\.previous\.filter\(\(log\) => log\.id !== result\.log\.id\),\s*\]\)/);
...
assert.match(enrichmentSource, /const lensReady = analyses\.some\(\(item\) => item\.log_id === pendingEnrichmentLogId\)/);
assert.match(enrichmentSource, /if \(lensReady\) \{\s*setEnrichmentPending\(false\);/);
...
assert.match(detailSource, /aria-label=\{\s*advancing \? "Reading next session" : "Read next session"\s*\}/);
assert.doesNotMatch(detailSource, /Saving(?:…| next session)/, "Read Today must use reader-facing wording while saving");

assert.match(llmSource, /export const NINE_ROUTER_MAX_RPS\s*=\s*positiveEnv\("NINE_ROUTER_MAX_RPS", 5, 100\)/);
assert.match(llmSource, /export const NINE_ROUTER_MAX_CONCURRENCY\s*=\s*positiveEnv\(\s*"NINE_ROUTER_MAX_CONCURRENCY",\s*30,\s*100,\s*\)/);
assert.match(llmSource, /export const NINE_ROUTER_BACKGROUND_CONCURRENCY\s*=\s*NINE_ROUTER_MAX_CONCURRENCY > 1[\s\S]*?NINE_ROUTER_MAX_CONCURRENCY - 1[\s\S]*?: 1/);
assert.match(llmSource, /export const NINE_ROUTER_DISPATCH_INTERVAL_MS\s*=\s*Math\.ceil\(\s*1_000 \/ NINE_ROUTER_MAX_RPS,?\s*\)/);
