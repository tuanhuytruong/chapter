...
assert.match(storyViewSource, /GlossaryTerm term="InnerMovement"/);
assert.match(storyViewSource, /GlossaryTerm term="Shift"/);
assert.match(storyViewSource, /GlossaryTerm term="Tension"/);
assert.match(storyViewSource, /GlossaryTerm term="WantVsAction"/);
assert.match(storyViewSource, /GlossaryTerm term="StillOpen"/);
assert.match(glossarySource, /InnerMovement:/);
assert.match(glossarySource, /WantVsAction:/);
...
assert.doesNotMatch(storyViewSource, /role="tablist"/);
...
const continuityCitation = parseStoryThreadAnalysis(JSON.stringify({ storyRecap: "x", storySoFar: "y", continuityPath: [{ text: "A turning point", citation: { logId: "fd059abf-c569-4366-ac88-789926f44978", session: 2, pageStart: 5, pageEnd: 5 } }], changedEvents: [], threads: [], characterPulse: [], readerMemory: [], confidenceNotes: [] }));
...
assert.match(storyViewSource, /view: "thread" \| "characters"/);
assert.match(storyViewSource, /readingRound: number/);
assert.match(storyViewSource, /Story companion · Round/);
assert.match(detailSourceX, /id="story-thread-tab"/);
assert.match(detailSourceX, /id="character-storylines-tab"/);
